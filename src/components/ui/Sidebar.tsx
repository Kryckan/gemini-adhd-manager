...
export function Sidebar() {
    ...
    return (
        <aside className="w-20 border-r border-neutral-900 flex flex-col items-center py-6 gap-6 z-10 bg-[#0A0A0A] shrink-0">
            ...
            <NavButton icon={<FolderClosed size={20} />} shortcut="P" label="Projects" />
            <NavButton icon={<Focus size={20} />} shortcut="F" label="Focus" active />
            <NavButton icon={<Calendar size={20} />} shortcut="M" label="Meetings" />
            <NavButton icon={<BarChart2 size={20} />} shortcut="A" label="Analytics" />
            <NavButton icon={<Settings size={20} />} shortcut="S" label="Settings" />
...
function NavButton({ icon, shortcut, label, active = false }: { icon: React.ReactNode, shortcut: string, label: string, active?: boolean }) {
    return (
        <button className={`relative w-full flex text-sm flex-col items-center justify-center transition-all py-3 group focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-cyan-500 rounded-none ${active ? 'bg-neutral-900/40 text-white' : 'text-neutral-500 hover:text-neutral-300 hover:bg-neutral-900/20'}`}>
            <div className={`mb-1 relative`}>{icon}</div>
            <span className={`text-[10px] font-medium tracking-wide mt-1 ${active ? 'text-neutral-300' : 'text-neutral-500'}`}>{label}</span>
            ...
            {active && <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-[#00A3FF] shadow-[0_0_12px_rgba(0,163,255,0.8)]" />}
        </button>
    );
}
